...
import { MdClose, FiMonitor, FiSmartphone, PiDiceFiveBold } from "~/components/icons";
...
function Picker() {
  ...
  return (
    <div className="picker">
      ...
      <Topbar right={<a href="/" style={{ fontSize: 12, display: "inline-flex", alignItems: "center", gap: 4 }}><MdClose size={14} /> back to the room</a>} />
        ...
        <div className="sec">
          ...
          <div className="personas">
            ...
            <div className={`persona ${personaId === "custom" ? "on" : ""}`} onClick={() => setPersonaId("custom")} role="button">
              ...
              <span style={{ color: "#1f5fbf" }}>{cat.data?.features.bedrock ? "Bedrock builds the phone from this." : cat.data?.features.llm === "openai" ? "The local model builds the phone from this." : "Local mode: a template phone is built from this."}</span>
            </div>
          </div>
        </div>
        <div className="sec">
        ...
        <div className="sec">
          ...
          <div className="devices">
            {([["auto", "Follows the case", `${family === "surprise" ? "Decided when the drill starts" : `${famDevice === "laptop" ? "Windows laptop" : "Android phone"} for this case`}`], ["phone", "Android phone", "One UI · WhatsApp, Messages, Bank, PhonePe"], ["laptop", "Windows laptop", "Windows 11 · Chrome, Gmail, Meet, AnyDesk"]] as const).map(([id, b, s]) => (
              <button key={id} className={`device ${device === id ? "on" : ""}`} onClick={() => setDevice(id)}><span style={{ fontSize: 22, display: "grid", placeItems: "center" }}>{id === "laptop" ? <FiMonitor size={22} /> : id === "phone" ? <FiSmartphone size={22} /> : <PiDiceFiveBold size={22} />}</span><div><b>{b}</b><span>{s}</span></div></button>
            ))}
          </div>
        </div>
        <div className="foot">
          <div className="opts">
            ...
            <span style={{ color: "#6b5636" }}>Mode: {cat.data?.mode === "aws" ? "AWS (Bedrock · Transcribe · Polly)" : cat.data?.features.llm === "openai" ? "local (local model caller · Piper · Vosk)" : "local (scripted caller, browser voice)"}</span>
          </div>
          <button className="px-btn primary" disabled={!canGo || create.isPending || !cat.data} onClick={() => create.mutate()}>
            {create.isPending ? "Building the device…" : effDevice === "laptop" ? <><FiMonitor size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} />Put the laptop on the desk</> : <><FiSmartphone size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} />Put the phone on the desk</>}
          </button>
        </div>
