...
import type { DrillSummary, Ending, ServerEvent, Tactic, UiHint } from "~/shared/types";
...
interface DrillState {
  ...
  hint: UiHint;
  phase: string;
  voice: { tts: "polly" | "piper" | "browser" | "captions"; stt: "transcribe" | "browser" | "typed"; note?: string };
...
let idc = 1;
const initial = { drill: null, connected: false, callState: "idle" as const, chat: [] as ChatMsg[], notifs: [] as Notif[], transcript: [] as DrillState["transcript"], partial: "", scammerSpeaking: false, lastScammerLine: "", familyCall: null, tripped: false, ended: null, packetUrl: null, packetPercent: 0, moves: [] as DrillState["moves"], index: 0, hint: null as UiHint, phase: "", voice: { tts: "captions" as const, stt: "typed" as const } };
...
export const useDrill = create<DrillState>((set) => ({
  ...
  apply: (e) => {
    switch (e.type) {
      ...
      case "breaker.trip": set({ tripped: true, index: e.index, hint: null }); break;
